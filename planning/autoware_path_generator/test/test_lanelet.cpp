...
TEST_F(UtilsTest, getLaneletsWithinRouteUpTo)
{
  ...
  {  // Normal case
    ...
    const auto prev_lanelets =
      utils::get_lanelets_within_route_up_to(lanelet, *route_manager_, distance);
  ...
  {  // The given lanelet is not within the route
    const auto lanelet = get_lanelet_from_id(10257);
    const auto distance = 30.0;
...
TEST_F(UtilsTest, getLaneletsWithinRouteAfter)
{
  ...
  {  // Normal case
    const auto lanelet = get_lanelet_from_id(125);
    const auto distance = 30.0;

    const auto next_lanelets =
      utils::get_lanelets_within_route_after(lanelet, *route_manager_, distance);
