...
protected:
  void SetUp() override
  {
    ...
    node_options.parameter_overrides().emplace_back(
      "obstacle_stop.obstacle_filtering.check_inside.pointcloud", true);

    node_ = std::make_unique<rclcpp::Node>("test_node", node_options);
...
TEST_F(PlanningFactorTest, TestWithPointCloudObstacle)
{
  InitializeEgoData();
  ...
  const auto count_param_str =
    "obstacle_stop.pointcloud_segmentation.velocity_estimation.required_velocity_count";
