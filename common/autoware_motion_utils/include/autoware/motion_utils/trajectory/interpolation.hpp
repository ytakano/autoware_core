...
geometry_msgs::msg::Pose calcInterpolatedPose(const T & points, const double target_length)
{
  ...
  if (points.size() < 2 || target_length < 0.0) {
    return autoware_utils_geometry::get_pose(points.front());
  }

  double accumulated_length = 0;
  for (size_t i = 0; i < points.size() - 1; ++i) {
    const auto & curr_pose = autoware_utils_geometry::get_pose(points.at(i));
    const auto & next_pose = autoware_utils_geometry::get_pose(points.at(i + 1));
  ...
  return autoware_utils_geometry::get_pose(points.back());
}
