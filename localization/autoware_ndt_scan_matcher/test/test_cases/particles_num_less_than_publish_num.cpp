...
TEST_F(TestNDTScanMatcherLowParticleNum, particle_num_less_than_publish_num)  // NOLINT
{
  ...
  EXPECT_EQ(trace_events[1].particles_requested, 10);
  EXPECT_EQ(trace_events[1].particles_evaluated, 10);
  EXPECT_EQ(trace_events[1].marker_publish_count, 10);
  EXPECT_EQ(trace_events[1].cloud_publish_count, 10);
