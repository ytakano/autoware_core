...
// were moved to estimate_covariance_math.cpp so they can be swapped for the Rust port via the
// NDT_USE_RUST build switch. The functions below stay in C++ because they drive the NDT engine
// (point clouds / align); they still call the pure helpers through estimate_covariance.hpp.
