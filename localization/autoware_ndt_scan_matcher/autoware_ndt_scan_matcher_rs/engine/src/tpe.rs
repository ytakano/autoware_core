...
/// Dimension of a TPE trial input vector (the six pose-search coordinates). Public so the node
/// crate's C-ABI shims can validate the caller's buffer length against it.
pub const INPUT_DIMENSION: usize = 6;
/// Dimension of the prior mean/stddev vectors. Public so the node crate's C-ABI shims can validate
/// the caller's buffer length against it.
pub const PRIOR_DIMENSION: usize = 5;
